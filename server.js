...
function defaultData() {
  return {
    config: {
      ...
      championLock: "tournament_start", // "tournament_start" | "first_submission"
...
function postChatBot(text) {
  if (!Array.isArray(db.chat)) db.chat = [];
  db.chat.push({ id: uid(), person_id: null, system: true, text, created_at: now().toISOString() });
  if (db.chat.length > 2000) db.chat = db.chat.slice(-2000);
}
...
async function syncFromFootballData() {
  ...
  try {
    const sr = await fetch(`https://api.football-data.org/v4/competitions/${comp}/standings`, { headers: { "X-Auth-Token": key } });
...
function publicPerson(p) {
  return p ? { id: p.id, display_name: p.display_name, joined_at: p.joined_at, hasPin: !!p.pin, avatar: p.avatar || "⚽", photo: p.photo || null } : null;
}
...
function gateToken() {
  ...
  return crypto.createHmac("sha256", db.config.gateSecret).update(currentAccessCode() || "").digest("hex").slice(0, 32);
}
function gateOk(req) {
  if (!currentAccessCode()) return true; // gate disabled
  return (req.headers["x-gate-token"] || "") === gateToken();
}
...
function chatView() {
  ...
  return (db.chat || [])
    ...
    .map((m) => m.system
      ? { id: m.id, system: true, text: m.text, created_at: m.created_at, reactions: reactionsFor(`chatmsg:${m.id}`) }
      : { id: m.id, person_id: m.person_id, name: nameById.get(m.person_id) || "Unknown", photo: photoById.get(m.person_id) || null, text: m.text, created_at: m.created_at, reactions: reactionsFor(`chatmsg:${m.id}`) });
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === "/api/gate" && req.method === "POST") {
      ...
      if ((code || "").trim() === currentAccessCode()) return send(res, 200, { ok: true, token: gateToken() });
    ...
    if (p === "/api/join" && req.method === "POST") {
      ...
      if (cleanPin && !/^\d{4,8}$/.test(cleanPin)) return send(res, 400, { error: "PIN must be 4–8 digits." });
      ...
      if (existing) {
        ...
        if (existing.pin) {
          if (cleanPin !== existing.pin) return send(res, 401, { error: "Wrong PIN for that name." });
        } else if (cleanPin) {
          existing.pin = cleanPin; // first time adding a PIN to protect the name
          save();
        }
        return send(res, 200, { person: publicPerson(existing), returning: true });
      }
      // New player.
      const person = { id: uid(), display_name: clean, joined_at: now().toISOString(), pin: cleanPin || null };
      db.people.push(person);
      save();
      return send(res, 200, { person: publicPerson(person), returning: false });
    }
    ...
    // --- current matches (today + tomorrow) ---
    if (p === "/api/matches" && req.method === "GET") {
      const personId = q.get("personId");
      const list = nextMatchDayMatches().map((m) => matchView(m, personId));
    ...
    if (p === "/api/chat" && req.method === "GET") {
      ...
      const typing = Object.keys(typingMap)
        ...
        .filter(Boolean);
      return send(res, 200, { messages: chatView(), typing, serverTime: now().toISOString() });
    }
    ...
    if (p === "/api/chat" && req.method === "POST") {
      const { personId, text } = await readBody(req);
      const person = getPerson(personId);
      if (!person) return send(res, 404, { error: "Unknown player." });
      ...
      db.chat.push({ id: uid(), person_id: personId, text: clean, created_at: now().toISOString() });
    ...
    if (p === "/api/profile/update" && req.method === "POST") {
      const { personId, pin, newName, newPin } = await readBody(req);
      const person = getPerson(personId);
      if (!person) return send(res, 404, { error: "Unknown player." });
      if (person.pin && String(pin || "") !== person.pin) return send(res, 401, { error: "Wrong current PIN." });
    ...
    const requireAdmin = () => adminKey === ADMIN_KEY;
    ...
    if (p === "/api/admin/photo" && req.method === "POST") {
      ...
      person.photo = photo || null; // URL or data URI; null clears it
      save();
